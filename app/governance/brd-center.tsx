...
import { brdLifecycleFilters, brdSectionTemplates } from "./brd-contract";

type DocumentSummary = {
  id: string; businessId: string; productId: string; productName: string; projectId: string | null; projectName: string | null;
...
export function BrdCenter({ canCreate, canEdit, canVersion, canSubmit }: { canCreate: boolean; canEdit: boolean; canVersion: boolean; canSubmit: boolean }) {
  ...
  const [activeSection, setActiveSection] = useState(brdSectionTemplates[0].key as string);
  ...
  const loadDocuments = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/v3/documents?${params}`, { headers: { accept: "application/json" } });
      const body = await response.json() as { data?: DocumentSummary[]; meta?: { total: number }; error?: { message?: string } };
      if (!response.ok) throw new Error(body.error?.message ?? "BRDs could not be loaded.");
      setDocuments(body.data ?? []); setTotal(body.meta?.total ?? 0);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "BRDs could not be loaded."); }
    finally { setLoading(false); }
  }, [q, status]);

  useEffect(() => { const handle = window.setTimeout(loadDocuments, 180); return () => window.clearTimeout(handle); }, [loadDocuments]);
  useEffect(() => {
  ...
  const loadWorkspace = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (!response.ok || !body.data) throw new Error(body.error?.message ?? "The BRD could not be opened.");
      setWorkspace(body.data); setActiveSection(body.data.sections[0]?.sectionKey ?? brdSectionTemplates[0].key);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "The BRD could not be opened."); }
  ...
  const saveMetadata = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch(editingMetadata && workspace ? `/api/v3/documents/${workspace.document.id}` : "/api/v3/documents", {
        ...
        body: JSON.stringify({ ...metadata, projectId: metadata.projectId || null, ...(editingMetadata && workspace ? { version: workspace.document.version } : {}) }),
      });
      const body = await response.json() as { data?: Workspace; error?: { message?: string; validationDetails?: Record<string, string> } };
      if (!response.ok || !body.data) { setFieldErrors(body.error?.validationDetails ?? {}); throw new Error(body.error?.message ?? "The BRD could not be saved."); }
      setWorkspace(body.data); setMetadataOpen(false); setSuccess(editingMetadata ? "BRD details updated." : "BRD created with all required sections."); await loadDocuments();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "The BRD could not be saved."); }
  ...
  const saveSections = async () => {
    ...
    try {
      ...
      const body = await response.json() as { data?: Workspace; error?: { message?: string } };
      if (!response.ok || !body.data) throw new Error(body.error?.message ?? "The BRD could not be saved.");
      setWorkspace(body.data); setSuccess("BRD draft saved."); await loadDocuments();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "The BRD could not be saved."); }
    finally { setSaving(false); }
  };

  const submitForReview = async () => {
    if (!workspace || !window.confirm("Submit this BRD for review? Its section content will be locked and further changes will require a revision.")) return;
    setSaving(true); setError("");
    try {
      ...
      const body = await response.json() as { data?: Workspace; error?: { message?: string; validationDetails?: Record<string, string> } };
      if (!response.ok || !body.data) throw new Error(body.error?.message ?? "The BRD could not be submitted.");
      setWorkspace(body.data); setSuccess("BRD submitted as an immutable review version."); await loadDocuments();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "The BRD could not be submitted."); }
    finally { setSaving(false); }
  };
  ...
  const createRevision = async () => {
    ...
    try {
      ...
      setWorkspace(body.data); setRevisionSummary(""); setSuccess("BRD revision created."); await loadDocuments();
  ...
  return <section className="brd-module" aria-labelledby="brd-title">
    <div className="portfolio-toolbar"><div><span className="section-kicker">STAGE 3 · GOVERNED DOCUMENTS</span><h2 id="brd-title">Business Requirements Documents <small>{total}</small></h2></div>{canCreate && !workspace && !workspaceLoading && <button className="primary-action" onClick={openCreate}>+ New BRD</button>}</div>
    ...
    {workspaceLoading ? <div className="module-state" role="status"><span className="loader" /> Opening BRD…</div> : !workspace ? <>
      <div className="filter-bar" aria-label="BRD filters"><label className="search-field"><span className="sr-only">Search BRDs</span><input value={q} onChange={(event) => setQ(event.target.value)} placeholder="Search BRD title or ID" /></label><label><span className="sr-only">Filter by lifecycle</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All lifecycle states</option>{brdLifecycleFilters.map((value) => <option key={value}>{value}</option>)}</select></label>{(q || status) && <button className="text-action" onClick={() => { setQ(""); setStatus(""); }}>Clear</button>}</div>
      {loading ? <div className="module-state" role="status"><span className="loader" /> Loading BRDs…</div>
        : documents.length === 0 ? <div className="module-state empty-state"><span>▤</span><strong>No BRDs found</strong><p>{q || status ? "Change or clear the current filters." : "Create the first governed Business Requirements Document."}</p>{canCreate && !q && !status && <button onClick={openCreate}>Create BRD</button>}</div>
        : <div className="table-shell"><table className="portfolio-table"><thead><tr><th>BRD</th><th>Product / Project</th><th>Version</th><th>Lifecycle</th><th>Owner</th><th>Updated</th></tr></thead><tbody>{documents.map((document) => <tr key={document.id} className="clickable-row" onClick={() => void loadWorkspace(document.id)}><td><button className="table-link" onClick={(event) => { event.stopPropagation(); void loadWorkspace(document.id); }}>{document.title}</button><span>{document.businessId}</span></td><td><strong>{document.productName}</strong><span>{document.projectName ?? "Product-wide"}</span></td><td>{document.versionLabel ?? "—"}</td><td><span className="neutral-badge">{document.lifecycleStatus ?? "Unavailable"}</span></td><td>{document.ownerName ?? "Unassigned"}</td><td>{new Date(document.updatedAt).toLocaleDateString()}</td></tr>)}</tbody></table></div>}
    </> : <div className="brd-workspace">
      <header className="brd-document-header"><div><button className="text-action" onClick={() => setWorkspace(null)}>← Back to BRD library</button><span className="section-kicker">{workspace.document.businessId} · {workspace.document.versionLabel}</span><h3>{workspace.document.title}</h3><p>{workspace.document.purpose || "No purpose statement has been recorded."}</p><div className="brd-meta"><span>{workspace.document.productName}</span><span>{workspace.document.projectName ?? "Product-wide"}</span><span>{workspace.document.ownerName ?? "Unassigned owner"}</span><span className="neutral-badge">{workspace.document.lifecycleStatus}</span></div></div><div className="brd-header-actions">{isDraft && canEdit && <button className="secondary-action" onClick={openEditMetadata}>Edit details</button>}{isDraft && canEdit && <button className="secondary-action" onClick={() => void saveSections()} disabled={saving}>{saving ? "Saving…" : "Save draft"}</button>}{isDraft && canSubmit && <button className="primary-action" onClick={() => void submitForReview()} disabled={saving || completed !== workspace.sections.length}>Submit for review</button>}</div></header>
      ...
      <div className="brd-authoring-grid"><nav className="brd-section-nav" aria-label="BRD sections">{workspace.sections.map((section) => <button key={section.sectionKey} className={section.sectionKey === activeSection ? "active" : ""} onClick={() => setActiveSection(section.sectionKey)} aria-current={section.sectionKey === activeSection ? "page" : undefined}><span>{String(section.sequence).padStart(2, "0")}</span><strong>{section.heading}</strong><i className={`section-state state-${section.completionStatus.toLowerCase()}`} aria-label={section.completionStatus}>{section.completionStatus === "COMPLETE" ? "✓" : section.completionStatus === "IN_PROGRESS" ? "•" : "○"}</i></button>)}</nav>
    ...
    {metadataOpen && <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setMetadataOpen(false); }}><div className="product-dialog brd-dialog" role="dialog" aria-modal="true" aria-labelledby="brd-dialog-title"><form onSubmit={saveMetadata}><div className="dialog-header"><div><span className="section-kicker">{editingMetadata ? workspace?.document.businessId : "NEW GOVERNED DOCUMENT"}</span><h2 id="brd-dialog-title">{editingMetadata ? "Edit BRD details" : "Create Business Requirements Document"}</h2></div><button type="button" aria-label="Close" onClick={() => setMetadataOpen(false)} disabled={saving}>×</button></div><div className="form-grid">
