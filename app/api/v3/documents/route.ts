...
import { governanceLifecycleFilters, parseGovernanceDocumentType, validateGovernanceDocumentInput } from "../../../governance/brd-contract";
...
export async function GET(request: Request) {
  ...
  const requestedType = url.searchParams.get("documentType") ?? "";
  const documentType = requestedType ? parseGovernanceDocumentType(requestedType) : "";
  if (requestedType && !documentType) return apiError(422, "VALIDATION_FAILED", "Select a valid document type.", context.correlationId, context.timestamp, { documentType: "Only BRD or PRD is supported." });
  if (requestedStatus && !governanceLifecycleFilters.includes(requestedStatus as typeof governanceLifecycleFilters[number])) return apiError(422, "VALIDATION_FAILED", "Select a valid document status.", context.correlationId, context.timestamp, { status: "Status is not supported." });
  const { listGovernanceDocuments } = await import("../../../../db/governance-documents");
  const result = await listGovernanceDocuments({
    documentType,
...
export async function POST(request: Request) {
  ...
  const documentType = source.documentType === undefined ? "BRD" : parseGovernanceDocumentType(source.documentType);
  if (!documentType) return apiError(422, "VALIDATION_FAILED", "Select a valid document type.", context.correlationId, context.timestamp, { documentType: "Only BRD or PRD is supported." });
  ...
  const { createGovernanceDocument } = await import("../../../../db/governance-documents");
  try {
    const result = await createGovernanceDocument(documentType, validation.value, context.principal.user.userId, context.correlationId);
  ...
  } catch { return apiError(500, "DOCUMENT_CREATE_FAILED", `The ${documentType} could not be created.`, context.correlationId, context.timestamp); }
}
