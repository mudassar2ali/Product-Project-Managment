...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [source, sprintSource] = await Promise.all([provider.getAdvancedWorkItems(sync.azureProjectId, sync.azureTeamId), sync.azureTeamId ? provider.getSprintHistory(sync.azureProjectId, sync.azureTeamId) : Promise.resolve(null)]);
