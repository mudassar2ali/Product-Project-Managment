...
export async function getSprint(id: string) {
  ...
  const baseline = await env.DB.prepare("SELECT item_count itemCount,committed_points committedPoints,committed_hours committedHours,membership_snapshot_json membershipSnapshotJson,activated_at activatedAt FROM sprint_baselines WHERE sprint_id=?").bind(id).first();
  const dispositions = await env.DB.prepare("SELECT d.membership_id membershipId,d.disposition,d.target_sprint_id targetSprintId,target.business_id targetBusinessId,target.name targetName,d.reason FROM sprint_completion_dispositions d LEFT JOIN sprints target ON target.id=d.target_sprint_id WHERE d.sprint_id=? ORDER BY d.created_at").bind(id).all();
  return { kind: "ok" as const, sprint, memberships: memberships.results, available: available.results, baseline, dispositions: dispositions.results };
}
