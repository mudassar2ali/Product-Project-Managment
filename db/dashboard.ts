...
export async function getDashboard(access: DeliveryInsightAccess = { backlog: true, sprints: true, metrics: true }) {
  const one = async (sql: string) => env.DB.prepare(sql).first<Record<string, number>>();
  const [portfolio, health, milestones, raid, ideas, progress, attention, delivery] = await Promise.all([
  ...
  return {
    ...
    progress,
    delivery,
    attention: [...delivery.attention, ...attention.results].slice(0, 12),
