...
import { brdSectionTemplates, type BrdDocumentInput, type BrdSectionInput } from "../app/governance/brd-contract";

type DocumentRow = {
  id: string; businessId: string; documentType: "BRD"; productId: string; productName: string;
...
type SectionRow = BrdSectionInput & { id: string; documentVersionId: string; version: number; updatedAt: string };
...
async function nextBusinessId() {
  const result = await env.DB.prepare(`
    INSERT INTO business_sequences(entity_type,next_value,updated_at) VALUES('BRD',2,CURRENT_TIMESTAMP)
    ON CONFLICT(entity_type) DO UPDATE SET next_value=next_value+1,updated_at=CURRENT_TIMESTAMP
    RETURNING next_value-1 value
  `).first<{ value: number }>();
  return `BRD-${String(result?.value ?? 1).padStart(4, "0")}`;
}

async function validateScope(input: BrdDocumentInput) {
...
export async function listBrdDocuments(input: { q: string; productId: string; projectId: string; status: string; page: number; pageSize: number }) {
  const conditions = ["d.document_type='BRD'", "d.record_status='ACTIVE'"];
...
export async function getBrdWorkspace(id: string) {
  const document = await env.DB.prepare(`
    ...
    LEFT JOIN users u ON u.id=d.owner_user_id
    LEFT JOIN governance_document_versions v ON v.id=(SELECT cv.id FROM governance_document_versions cv WHERE cv.document_id=d.id ORDER BY ${currentVersionOrder.replaceAll("lifecycle_status", "cv.lifecycle_status").replaceAll("major_version", "cv.major_version").replaceAll("minor_version", "cv.minor_version").replaceAll("created_at", "cv.created_at")} LIMIT 1)
    WHERE d.id=? AND d.document_type='BRD' AND d.record_status='ACTIVE'
...
export async function getBrdVersionSections(versionId: string) {
  const version = await env.DB.prepare(`SELECT v.id,v.document_id documentId,v.version_label versionLabel,v.lifecycle_status lifecycleStatus,v.version,d.document_type documentType FROM governance_document_versions v JOIN governance_documents d ON d.id=v.document_id WHERE v.id=? AND d.record_status='ACTIVE'`).bind(versionId).first<{ id: string; documentId: string; versionLabel: string; lifecycleStatus: string; version: number; documentType: string }>();
  if (!version || version.documentType !== "BRD") return { kind: "not_found" as const };
...
export async function createBrd(input: BrdDocumentInput, actor: string, correlationId: string) {
  ...
  const statements = [
    env.DB.prepare(`INSERT INTO governance_documents(id,business_id,document_type,product_id,project_id,title,owner_user_id,purpose,created_by,updated_by) VALUES(?,?,'BRD',?,?,?,?,?,?,?)`).bind(id, businessId, input.productId, input.projectId, input.title, actor, input.purpose, actor, actor),
    env.DB.prepare(`INSERT INTO governance_document_versions(id,document_id,version_label,major_version,minor_version,lifecycle_status,change_summary,created_by,updated_by) VALUES(?,?,'Draft 0.1',0,1,'DRAFT','Initial BRD draft',?,?)`).bind(versionId, id, actor, actor),
    ...brdSectionTemplates.map((section, index) => env.DB.prepare(`INSERT INTO governance_document_sections(id,document_version_id,section_key,heading,sequence,content_text,required,completion_status,created_by,updated_by) VALUES(?,?,?,?,?,'',1,'EMPTY',?,?)`).bind(crypto.randomUUID(), versionId, section.key, section.heading, index + 1, actor, actor)),
    env.DB.prepare(`INSERT INTO audit_logs(id,entity_type,entity_id,action,after_json,actor_user_id,source,correlation_id,occurred_at) VALUES(?,?,?,?,?,?,'APPLICATION',?,CURRENT_TIMESTAMP)`).bind(crypto.randomUUID(), "GovernanceDocument", id, "CREATE", JSON.stringify({ businessId, documentType: "BRD", productId: input.productId, projectId: input.projectId, title: input.title, versionLabel: "Draft 0.1" }), actor, correlationId),
  ];
  await env.DB.batch(statements);
  return getBrdWorkspace(id);
}

export async function updateBrdMetadata(id: string, input: BrdDocumentInput, expectedVersion: number, actor: string, correlationId: string) {
  const scope = await validateScope(input);
  if (scope !== "ok") return { kind: scope };
  const before = await getBrdWorkspace(id);
  if (before.kind === "not_found") return before;
  if (before.document.lifecycleStatus !== "DRAFT") return { kind: "locked" as const };
  const results = await env.DB.batch([
    env.DB.prepare(`INSERT INTO audit_logs(id,entity_type,entity_id,action,before_json,after_json,actor_user_id,source,correlation_id,occurred_at) SELECT ?,?,?,?,?,?,'APPLICATION',?,CURRENT_TIMESTAMP WHERE EXISTS(SELECT 1 FROM governance_documents WHERE id=? AND version=? AND document_type='BRD' AND record_status='ACTIVE' AND EXISTS(SELECT 1 FROM governance_document_versions WHERE document_id=? AND lifecycle_status='DRAFT'))`).bind(crypto.randomUUID(), "GovernanceDocument", id, "UPDATE", JSON.stringify({ productId: before.document.productId, projectId: before.document.projectId, title: before.document.title, purpose: before.document.purpose }), JSON.stringify(input), actor, correlationId, id, expectedVersion, id),
    env.DB.prepare(`UPDATE governance_documents SET product_id=?,project_id=?,title=?,purpose=?,version=version+1,updated_at=CURRENT_TIMESTAMP,updated_by=? WHERE id=? AND version=? AND document_type='BRD' AND record_status='ACTIVE' AND EXISTS(SELECT 1 FROM governance_document_versions WHERE document_id=? AND lifecycle_status='DRAFT')`).bind(input.productId, input.projectId, input.title, input.purpose, actor, id, expectedVersion, id),
  ]);
  if (!results[1]?.meta.changes) return { kind: "conflict" as const };
  return { kind: "ok" as const, workspace: await getBrdWorkspace(id) };
}

export async function replaceBrdSections(versionId: string, sections: BrdSectionInput[], expectedVersion: number, actor: string, correlationId: string) {
  const current = await env.DB.prepare(`SELECT v.id,v.document_id documentId,v.lifecycle_status lifecycleStatus,v.version,d.document_type documentType FROM governance_document_versions v JOIN governance_documents d ON d.id=v.document_id WHERE v.id=? AND d.record_status='ACTIVE'`).bind(versionId).first<{ id: string; documentId: string; lifecycleStatus: string; version: number; documentType: string }>();
  if (!current || current.documentType !== "BRD") return { kind: "not_found" as const };
  if (current.lifecycleStatus !== "DRAFT") return { kind: "locked" as const };
  const keys = sections.map((section) => section.sectionKey);
  const statements = [
    env.DB.prepare(`INSERT INTO audit_logs(id,entity_type,entity_id,action,after_json,actor_user_id,source,correlation_id,occurred_at) SELECT ?,?,?,?,?,?,'APPLICATION',?,CURRENT_TIMESTAMP WHERE EXISTS(SELECT 1 FROM governance_document_versions WHERE id=? AND version=? AND lifecycle_status='DRAFT')`).bind(crypto.randomUUID(), "GovernanceDocumentVersion", versionId, "SECTIONS_REPLACE", JSON.stringify({ sectionCount: sections.length, completedSections: sections.filter((section) => section.completionStatus === "COMPLETE").length, contentCharacters: sections.reduce((sum, section) => sum + section.contentText.length, 0) }), actor, correlationId, versionId, expectedVersion),
  ...
  return { kind: "ok" as const, workspace: await getBrdWorkspace(current.documentId) };
}
...
export async function submitBrdVersion(versionId: string, expectedVersion: number, actor: string, correlationId: string) {
  ...
  if (rows.results.length !== brdSectionTemplates.length || incomplete.length) return { kind: "incomplete" as const, sections: incomplete.map((section) => section.sectionKey) };
  ...
  const results = await env.DB.batch([
    env.DB.prepare(`INSERT INTO audit_logs(id,entity_type,entity_id,action,after_json,actor_user_id,source,correlation_id,occurred_at) SELECT ?,?,?,?,?,?,'APPLICATION',?,CURRENT_TIMESTAMP WHERE EXISTS(SELECT 1 FROM governance_document_versions WHERE id=? AND version=? AND lifecycle_status='DRAFT')`).bind(crypto.randomUUID(), "GovernanceDocumentVersion", versionId, "SUBMIT_REVIEW", JSON.stringify({ versionLabel, contentHash, sectionCount: rows.results.length }), actor, correlationId, versionId, expectedVersion),
  ...
  if (!results[1]?.meta.changes) return { kind: "conflict" as const };
  return { kind: "ok" as const, workspace: await getBrdWorkspace(version.documentId) };
}

export async function createBrdRevision(documentId: string, changeSummary: string, actor: string, correlationId: string) {
  const workspace = await getBrdWorkspace(documentId);
  ...
  const baseSections = await env.DB.prepare(`SELECT section_key sectionKey,heading,sequence,content_text contentText,required,completion_status completionStatus FROM governance_document_sections WHERE document_version_id=? ORDER BY sequence`).bind(base.id).all<BrdSectionInput>();
  await env.DB.batch([
