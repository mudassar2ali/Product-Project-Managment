...
test("release navigation preserves Stage 2 capability and keeps unauthorized future modules absent", async () => {
  ...
  for (const featureLabel of ["Dashboard", "Products", "Projects", "Backlog", "Sprints", "Milestones", "Ideas", "RAID", "Reports", "Integrations", "Audit Trail", "BRD / PRD"]) assert.match(shell, new RegExp(`label: "${featureLabel.replaceAll("/", "\\/")}"`));
  for (const deferred of ["Administration", "Requirements", "Market Intelligence", "TAM", "SAM", "SOM", "Financial", "Scorecards", "Release Management", "AI Assistant"]) assert.doesNotMatch(shell, new RegExp(`label: "${deferred}"`));
