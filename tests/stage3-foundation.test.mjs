...
test("authorized document and requirement modules are exposed while unfinished Stage 3 modules remain absent", async () => {
  const source = await read("app/command-center-shell.tsx");
  assert.match(source, /label: "BRD \/ PRD"/);
  assert.match(source, /label: "Requirements"/);
